...
execute as @e[type=marker,tag=sat.building.placer] run execute at @s if block ~ ~-.25 ~ #minecraft:sculk_replaceable run function bro:invasion_generic/building_spawn/central_building
...
execute as @e[type=marker,tag=sat.building.placer] run schedule function bro:invasion_generic/building_spawn/central_building_tick 1 replace
